...
export const Top = (): JSX.Element => {
  ...
  const content = {
    ja: {
      ...
      missionDescription: "「シェフ・イン・レジデンス」とは、地域外のシェフを招き入れて地域の活性化を促す取り組みであり、地域の創造性や活力を高めるための重要な手段となります。特に、わたしたちは、「旅するシェフ」を地域活性化の鍵となる存在として捉え、わたしたちが目指す「シェフ・イン・レジデンス」の理想的なかたちの条件として以下の３つを挙げています。そして、全国各地でこうした活動が持続的に行われ、広がっていくよう応援しています。",
      awardTitle: "AWARD",
  ...
  return (
    <main className="w-full bg-[#f7e489] min-h-screen">
      ...
      {/* Hero Section */}
      <section className="relative bg-[url('/figmaAssets/group-4.png')] bg-cover bg-center bg-no-repeat pt-32 md:pt-36 lg:pt-40 scroll-mt-[80px] md:scroll-mt-[96px] transition-all duration-300 ease-in-out">
        <div className="aspect-[16/9] md:aspect-[21/9] transition-all duration-300 ease-in-out"></div>
      </section>
      ...
      <section className="relative w-full bg-[#71b0ff] min-h-[400px] sm:min-h-[500px] md:min-h-[600px] lg:min-h-[700px] xl:min-h-[800px] flex items-center justify-center scroll-mt-[80px] md:scroll-mt-[96px]" id="about">
        <img 
          ...
          className="pointer-events-none absolute inset-0 mx-auto my-auto w-full max-w-[500px] sm:max-w-[600px] md:max-w-[700px] lg:max-w-[800px] xl:max-w-[900px] h-auto object-contain transition-all duration-300 ease-in-out"
              ...
              <div>
                ...
                <div className="text-[#5a3729] text-base leading-6 space-y-4">
                  {content[language].awardDescription.split('\n').map((line, index) => {
                    ...
                    // Show form link after "アワードへのエントリーは下記フォームより受け付けます。" or "Entries (self-nominations or third-party nominations) are accepted via the form below."
                    if (line === 'アワードへのエントリーは下記フォームより受け付けます。' || line === 'Entries (self-nominations or third-party nominations) are accepted via the form below.') {
                      return (
                        <div key={index}>
